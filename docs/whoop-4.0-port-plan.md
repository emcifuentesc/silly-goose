...
> **Note (toolchain) ✅ pinned:** the crate pins `rust-version = 1.94`; this machine's default is stable 1.88. Added `Rust/core/rust-toolchain.toml` (`channel = "1.94"`, targets host + `aarch64-apple-ios{,-sim}`). Verified: `cargo test` and the **Xcode build phase now pick 1.94 automatically** (simulator build succeeds with no `RUSTUP_TOOLCHAIN` override).
